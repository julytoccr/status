...
$this->end();
?>
<div id="tablas" style="display: none"></div>
<?php
$this->Html->css('estatusr');
$this->pageTitle=__('ejecucion_problema',array($problema['nombre']));
?>
...
<div id="problema_enunciado">
	...
	<?php
		...
		{
			echo $this->Html->link(__('copiar_datos_minitab_excel'), '/ejecuciones/minitab/' . $ejecucion_id, array('target'=>'_blank'));
		}
	?>
</div>

<?php echo $this->Form->create('Ejecucion', array('url' => array('controller'=>'ejecuciones', 'action' => 'correccion')))?>
		...
		<tr class="fila_pregunta">
			<td class="pregunta">
				...
				<?php
					...
					if ($preg['link1']) echo '<br/>' . $this->Html->link($preg['link1'],$preg['link1'], array("target" => "_blank"));
					if ($preg['link2']) echo '<br/>' . $this->Html->link($preg['link2'],$preg['link2'], array("target" => "_blank"));
				?>
			</td>
			<td class="respuesta">
				<?php
					if ($reintentos[$i]>0){
						if($sintaxis[$preg['sintaxis_respuesta_id']]['nombre'] == "Vector"){
							echo $this->Form->input("Ejecucion.respuesta$i",array('label'=>'','size'=>16, 'align'=>'right', 'maxlength'=>200));
						}
						else{
							echo $this->Form->input("Ejecucion.respuesta$i",array('label'=>'','size'=>9, 'align'=>'right', 'maxlength'=>200));
						}
					}
					else{
						echo $data['Ejecucion']["respuesta$i"];
					}
				?>
			</td>
		</tr>
...
<div align="center">
	...
	<?php echo $this->Form->end(__('correccion'))?>
...
<?php
	echo $this->Ajax->observeForm('EjecucionMostrarForm', 
		array(
