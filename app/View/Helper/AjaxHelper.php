 ...
 */
 //TODO Moze jednak to kiedys naprawics
	...
	/*******CUSTOM RICARDO*******/
	function ajax_link($title,$form_id,$link_id,$action,$update_field){
		$data = $this->Js->get("#".$form_id)->serializeForm( 
			array( 
				'isForm' => false, 
				'inline' => true) 
			); 
			
		$this->Js->get("#".$link_id)->event( 
		  'click', 
		  $this->Js->request( 
			array('action' => $action), 
			array( 
					'update' => "#".$update_field, // element to update 
					'data' => $data, 
					'async' => true, 
					'dataExpression'=>true, 
					'method' => 'POST'
		...
		$link = $this->Html->link($title,'javascript:void(0);',array('id'=>$link_id)); 
		echo $link.$this->Js->writeBuffer();
	}
	
	function ajax_sidebar_actions($form_id, $link_class){
		$jscode = <<<EOD
		<script>
		$(function(){
			$('a.$link_class').click(function(){
				...
				$.post(
					link.attr('href'), 
					$("#$form_id").closest("form").serialize(),
