...
id.asig = 248
if (!exists("param")) {   # login usuario; la asignatura es "Est. Salud. M0"
  param = -1
  resultado = "Por favor, escribe tu identificador de usuario"
} else {
  Q = paste0("select id from usuarios where login='", param, "';")
  ejec = dbGetQuery(con, Q)
  if (dim(ejec)[1]==1) {
     id.usu = ejec[1,1]
  } else {
     param = -1
     resultado = "Este identificador no existe"
     dbDisconnect(con)
  }
}
...
##-- Solo tests de M0
 G = datos1[grep('^M0.[1-9]$', datos1$test),]
...
##-- Número preguntas
 ...
 for (a in curris) {
    ...
    for (i in 1:Tests) {
        h = is.na(g[,i])
        j = g[!h, i]
	not[i] = ifelse(sum(j)>=30, 10, sum(j)/3.0)
 ...
 resultado = paste(css, Tab, "<p>Las celdas muestran las preguntas bien contestadas (alguna vez), las preguntas contestadas (sin contar repeticiones) y la nota provisional en base a un objetivo de 30 aciertos")
