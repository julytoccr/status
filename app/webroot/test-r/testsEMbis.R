...
css = "<head><style> body {font-family: sans-serif;} td {padding-left: 5px;} table, th, td {border: 1px solid black;}</style></head>"
...
##-- Número preguntas
 ...
 Tab = '<table><tr><th>User</th><th>Apellidos</th>'
 niveles = c()
 for (i in 1:Tests) {
 ...
 for (a in curris) {
    ...
    Tab = paste0(Tab, '<tr><td>', alcu$Login[k], '</td><td>', alcu$Nombre[k], '</td>')
    for (i in 1:Tests) {
        h = is.na(g[,i])
        j = g[!h, i]
	not[i] = ifelse(sum(j)>=35, 10, sum(j)/3.5)
        Tab = paste0(Tab, '<td>', sum(j), ' / ', length(j), ': ', round(not[i],1),'</td>')
    }
    Tab = paste0(Tab, '<td>', round(mean(not), 1), '</td></tr>')
 }
 Tab = paste0(Tab, '<tr><td>TOT. preg.</td><td></td>')
 for (i in 1:Tests) 
