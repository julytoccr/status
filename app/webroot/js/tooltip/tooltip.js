...
var Tooltip = {
	...
	init: function (activator)
	{
		...
		// Remove Link Hrefs
		//CUSTOM:ESTATUS We want links working
      ...
      if (activator.timer) {
		...
		if (event.pageY) {
			...
			if (document.body.scrollTop > document.documentElement.scrollTop) {
				...
				var _top = event.clientY + document.documentElement.scrollTop;
			}

			if (document.body.scrollLeft > document.documentElement.scrollLeft) {
				var _left = event.clientX + document.body.scrollLeft;
			} else {
				var _left = event.clientX + document.documentElement.scrollLeft;
			}
		}
		...
		if ((_left + parseInt(tooltipWidth)) > winWidth) {
			_left = winWidth - parseInt(tooltipWidth) * 1.2;
		}

		activator.Tooltip.style.top = (_top +20) + "px";
		activator.Tooltip.style.left = _left + "px";
    },
